...
public class MFQ {
	...
	void schedulling() { // MFQ용 스케쥴링
		...
		if(!(PresentProcess == null)) {
			PresentProcess.BurstTime -= CoreWork1;	// 현재 FCFS가 비어있지 않으면 bursttime에서 처리량 빼주기
			if (CoreWork1 == 1) elec += 1;		//현재 코어가 E Core인경우 소비전력  +1
			else elec += 3;						//현재 코어가 P Core인경우 소비전력  +1
		}
		if(!(PresentProcess2 == null)) {
			PresentProcess2.BurstTime -= CoreWork2;
			if (CoreWork2 == 1) elec += 1;		//현재 코어가 E Core인경우 소비전력  +1
			else elec += 3;
		}
	    if(!(PresentProcess3 == null)) {
	    	PresentProcess3.BurstTime -= CoreWork3;
	    	if (CoreWork3 == 1) elec += 1;		//현재 코어가 E Core인경우 소비전력  +1
			else elec += 3;
	    }
	    if(!(PresentProcess4 == null)) {
	    	PresentProcess4.BurstTime -= CoreWork4;
	    	if (CoreWork4 == 1) elec += 1;		//현재 코어가 E Core인경우 소비전력  +1
			else elec += 3;
	    }
	}
