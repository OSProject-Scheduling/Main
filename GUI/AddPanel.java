...
//      manager.information.AddAlgorithm(p9);
            ...
            if(isLockAlgorithm) {
            	...
            	if(isMDRQ) {
            		...
            		AlgorithmComboBox.setSelectedItem(SetAlgorithm);
						...
						if (AlgorithmComboBox.getSelectedItem() == "MDRQ") {
							if (s == null) {
								AlgorithmComboBox.setSelectedItem("FCFS");
								SetAlgorithm = "FCFS";
							} 
							else {
								AlgorithmComboBox.setSelectedItem(s);
								SetAlgorithm = s;
							}
						}
					}
               ...
               if (AlgorithmComboBox.getSelectedItem() == "MDRQ") { // MDRQ일 경우 다양한 요소 표시
                  if (SetAlgorithm != AlgorithmComboBox.getSelectedItem()) {
                     SetAlgorithm = AlgorithmComboBox.getSelectedItem().toString();
                     s= AlgorithmComboBox.getSelectedItem().toString();
                     isMFQChange = true;
   ...
   private class AddActionListener  implements ActionListener{            // add 클릭 시 액션 리스너
      public void actionPerformed(ActionEvent e) {
         ...
         if ((manager.algorithm == null) && (manager.mdrq == null)) { // 현재 실행 중인 알고리즘이 없을 때
         ...
         if(AlgorithmComboBox.getSelectedItem().toString() == "MDRQ") {
         ...
         for(int i = 0; i<AlgorithmList.size(); i++) {
             if(AddProcess.Name.equals(AlgorithmList.get(i).Name)) {
            	 JOptionPane.showMessageDialog(null, "Error: is Not Name", "Error", JOptionPane.INFORMATION_MESSAGE);
            	 return;
             }
         }
         ...
         if (Row <= 14) {
            ...
            if ((manager.algorithm == null) && (manager.mdrq == null)) { // 현재 실행 중인 알고리즘이 없을 때
					...
					if (AlgorithmComboBox.getSelectedItem().toString() == "MDRQ") { // MFQ인 경우
						isMDRQ = true;						
