...
public class RunPanel extends JPanel{
	...
	private void ComponentSetting() {
		...
		ECoreSpinner.addChangeListener(new ChangeListener() {
			...
			@Override
			public void stateChanged(ChangeEvent e) {
				if (Integer.parseInt(PCoreSpinner.getValue().toString())
						+ Integer.parseInt(ECoreSpinner.getValue().toString()) > 4) {
					ECoreSpinner.setValue(Integer.parseInt(ECoreSpinner.getValue().toString()) - 1);
				} else if (Integer.parseInt(PCoreSpinner.getValue().toString())
						+ Integer.parseInt(ECoreSpinner.getValue().toString()) < 1) {
					ECoreSpinner.setValue(Integer.parseInt(ECoreSpinner.getValue().toString()) + 1);
				}
				GhanttChartGUISetting();
			}
	...
	private void GhanttChartGUISetting() {
	...
	private class RunActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			if((manager.algorithm == null) && (manager.mfq == null)) {
				...
				for(int i =1; i<manager.addPanel.AlgorithmList.size(); i++) {
